/**
 * AddProductScreen.tsx  (Admin app)  — ĐÃ CẬP NHẬT
 * ─────────────────────────────────────────────────────────────
 * Thay đổi:
 *   • Phần chọn "Loại món ăn" đọc từ useCategories() thay vì
 *     hardcode LOAI_SP — tự động cập nhật khi thêm hạng mục mới
 * ─────────────────────────────────────────────────────────────
 */
...
export default function AddProductScreen({ navigation }) {
  ...
  const [sizes, setSizes] = useState(
    DEFAULT_SIZES.map((s) => ({ ...s, enabled: true })),
  );
  ...
  const pickSubImage = async () => {
    ...
    if (!result.canceled)
      setSubImages((prev) => [...prev, { uri: result.assets[0].uri }]);
  };
  ...
  const handleSubmit = async () => {
    if (!tensp.trim() || !loaisp || !gia.trim()) {
      Alert.alert("Lỗi", "Vui lòng nhập đầy đủ tên, loại và giá");
      return;
    }
    ...
    try {
      if (mainImageUri) {
        ...
        hinhanh = await uploadImageToCloudinary(mainImageUri);
      }
      for (let i = 0; i < subImages.length; i++) {
        setUploading(`Đang tải ảnh phụ ${i + 1}/${subImages.length}...`);
        images.push(await uploadImageToCloudinary(subImages[i].uri));
      }
      ...
      await addDoc(collection(db, "sanpham"), {
        ...
        sizes: sizes
          .filter((s) => s.enabled)
          .map((s) => ({ id: s.id, label: s.label, extraPrice: s.extraPrice })),
      });
    ...
    >
      <ScrollView style={styles.container} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.form}>
          ...
          {/* ── ẢNH PHỤ ── */}
          <Text style={styles.label}>Ảnh phụ (tối đa 4)</Text>
          ...
          <TextInput
            ...
            placeholder="Mô tả nguyên liệu, hương vị, đặc điểm..."
            value={mota}
          ...
          <Text style={styles.hint}>
            Nhấn để bật/tắt size. Giá = giá gốc + phụ thu size.
          </Text>
...
const styles = StyleSheet.create({
  ...
  categoryCardActive: { borderColor: "#3498db", backgroundColor: "#eaf4fd" },
  ...
  sizeCardActive: { backgroundColor: "#3498db", borderColor: "#3498db" },
